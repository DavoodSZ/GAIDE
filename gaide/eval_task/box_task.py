...
class BoxTask(Planning):
    ...
    def batch_clean_planning_problems(self, planning_configs, planning_difficulty: str = "free2free"):
        pairs = {
            ...
            "free2tight": [("table", "inner")],
            "tight2free": [("inner", "table")],
            "tight2tight": [("inner", "inner")],
        }
...
class PointCloudSampler:
    ...
    def sample(self, asset_dir: str, env_info: dict):
        ...
        cage_fp = os.path.join(asset_dir, "cage/cage.obj")
        pcd = np.concatenate([pcd, self._mesh_sample(cage_fp, env_info["cage"])], axis=0)
        return pcd
...
def make_world_config(ws_dir: str, env_info: dict, table_info: dict, robot):
    ...
    cage_pos = robot.config_in_base(
        points=torch.as_tensor(env_info["cage"][0], device=robot.device, dtype=torch.float32),
        table_info=table_info,
    ).tolist()
    cage_quat = [float(x) for x in _quaternion_from_rpy(*env_info["cage"][1])]
    world_config["mesh"]["cage"] = {
        "pose": cage_pos + cage_quat,
        "file_path": os.path.join(ws_dir, "cage/cage.obj"),
    }
