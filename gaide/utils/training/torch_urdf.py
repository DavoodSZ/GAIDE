...
'''
LINK: https://github.com/mihdalal/robofin/blob/192c500a801ae938586d49ccde37cbc6c29ae305/robofin/torch_urdf.py
'''

from .urchin_urdf import (
    URDF,
    URDFTypeWithMesh,
    Joint,
    Link,
...
from .urchin_utils import parse_origin 
...
class TorchJoint(Joint):
    ...
    def _rotation_matrices(self, angles, axis):
        '''
        NOTE: (Rodrigues' Rotation Formula)The rotation matrix can be calculated as follows:
        R = I + (sin(theta)).[K] + (1-cos(theta)).[K]^2

        Compute rotation matrices from (ARBITRARY) angle/axis representations.
        Parameters
        ----------
        angles : (n,) float
            The angles.
        ...
        NOTE: In NumPy, `np.newaxis` is used to increase the dimension of an existing 
        array by one.
        '''
        ...
        M[:, 2, 2] = cosa
        M[:, :3, :3] += (
            torch.outer(axis, axis).repeat((len(angles), 1, 1))        # [K]^2 = -I + outer (here -I negates the I in Rodrigues' formula)
            * (1.0 - cosa)[:, np.newaxis, np.newaxis]
        )
        M[:, :3, :3] += (
            torch.tensor(                                               # Skew-symmetric matrix K
                [
                    [0.0, -axis[2], axis[1]],
                    [axis[2], 0.0, -axis[0]],
                    [-axis[1], axis[0], 0.0],
    ...
    def get_child_poses(self, cfg, n_cfgs):
        ...
        if cfg is None:
            ...
            return torch.matmul(
                ...
                self._rotation_matrices(cfg, self.axis).type_as(cfg),                          # 240
            )
...
class TorchURDF(URDF):
    ...
    def load(file_obj, lazy_load_meshes=True, device=None):
        ...
        if isinstance(file_obj, str):                                               # YES, path to URDF.
            if os.path.isfile(file_obj):                                            # YES, There is a FILE.
                # Removes comments and blank texts (spapces, newline, ect) during parsing
                parser = ET.XMLParser(remove_comments=True, remove_blank_text=True)
                tree = ET.parse(file_obj, parser=parser)                            # Parses the URDF FILE
            ...
            path, _ = os.path.split(file_obj.name)                                  # Removes the URDF from the end.

        node = tree.getroot()
        '''
        # Example: Print out all joint names and their types
        for joint in node.findall('joint'):
            joint_name = joint.get('name')
            joint_type = joint.get('type')
            print(f'Joint Name: {joint_name}, Joint Type: {joint_type}')'''
    ...
    def _parse_simple_elements(cls, node, path, lazy_load_meshes, device):
        ...
        kwargs = {}
        for a in cls._ELEMENTS:
            t, r, m = cls._ELEMENTS[a]                    # (class, bool, bool)
            if not m:                                     # Not multiple (This will not be triggered here)
                v = node.find(t._TAG)
                if r or v is not None:
                    if issubclass(t, URDFTypeWithMesh):
                        v = t._from_xml(v, path, lazy_load_meshes)
                ...
                vs = node.findall(t._TAG)                                       # TAG name for each object in the HTML (LINK, JOINT, TRANSMISSION, MATERIAL)
                if len(vs) == 0 and r:
                    raise ValueError(
                        "Missing required subelement(s) of type {} when "
                        "parsing an object of type {}".format(t.__name__, cls.__name__)
    ...
    def _process_cfgs(self, cfgs):
        ...
        per joint.
        '''
        joint_cfg = {}                                                              # {'actuated joint (class)': value}
        assert isinstance(cfgs, torch.Tensor), "Incorrectly formatted config array"
        n_cfgs = len(cfgs)
        for i, j in enumerate(self.actuated_joints):                               # Number of actuated_joints is 8. Maybe one for the gripper is enough.
            joint_cfg[j] = cfgs[:, i]
    ...
    def link_fk_batch(self, cfgs=None, use_names=False):
        ...
        '''
        joint_cfgs, n_cfgs = self._process_cfgs(cfgs)                                     # 464

        # Process link set
        link_set = self.links                                                             # Panda has 14 links.

        # Compute FK mapping each link to a vector of matrices, one matrix per cfg
        fk = OrderedDict()
        for lnk in self._reverse_topo:                                                    # Enumerates links from base to end-effector.
            if lnk not in link_set:
                continue
            poses = torch.eye(4, device=self.device).repeat((n_cfgs, 1, 1))               # n_cfgs along rows, 1 along column, 1 along depth!
            path = self._paths_to_base[lnk]                                               # Kinematic chain from link to the base.
            for i in range(len(path) - 1):
                child = path[i]
                parent = path[i + 1]
                joint = self._G.get_edge_data(child, parent)["joint"]                     # A graph, nodes are LINKS and EDGES are JOINTs ()

                cfg_vals = None
                if joint.mimic is not None:                                               # One joint is a mimic joint, we have 9 in total and 8 actuated joint
                    mimic_joint = self._joint_map[joint.mimic.joint]
                    if mimic_joint in joint_cfgs:
                        cfg_vals = joint_cfgs[mimic_joint]
                        cfg_vals = (
                            joint.mimic.multiplier * cfg_vals + joint.mimic.offset       # Well, multiplier is 1 and offset is 0.
                        )
                elif joint in joint_cfgs:
                    cfg_vals = joint_cfgs[joint]

                child_poses = joint.get_child_poses(cfg_vals, n_cfgs)                    # 277 (Transformation matrices between two links.)
                poses = torch.matmul(child_poses, poses.type_as(child_poses))            # Sqeuntially calculates the transformations along the kinematic chain.
    ...
    def visual_geometry_fk_batch(self, cfgs=None):
        ...
        '''
        lfk = self.link_fk_batch(cfgs=cfgs)                                   # 481
