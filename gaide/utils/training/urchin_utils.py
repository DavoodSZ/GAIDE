...
'''
LINK: https://github.com/fishbotics/urchin/blob/main/urchin/utils.py
'''
...
def parse_origin(node):
    '''
    Find the origin subelement of an XML node and convert it into a 4x4 homogeneous transformation matrix.

    Args:
        node: class `lxlm.etree.Element`: An XML node which optionally has a child node with the ``origin``
        tag.

    Returns:
        matrix: The 4x4 homogeneous transform matrix that corresponds to this node's origin child.
    '''
...
def unparse_origin(matrix):
    ...
    Args:
        The 4x4 homogenous transform matrix to convert into an ``origin`` XML node.
