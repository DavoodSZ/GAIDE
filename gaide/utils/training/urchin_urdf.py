...
'''
LINK: https://github.com/fishbotics/urchin/blob/main/urchin/urdf.py
'''
...
from .urchin_utils import configure_origin, get_filename, load_meshes, parse_origin, unparse_origin
...
class URDFType:
    ...
    def _parse_simple_attribs(cls, node):
        ...
        for a in cls._ATTRIBS:
            ...
            if r:
                try:
                    v = cls._parse_attrib(t, node.attrib[a])                 # name of the robot is panda
                except Exception:
                    raise ValueError(f'Missing required attribute {a} when parsing an object of type {cls.__name__}')
            else:
                v = None
                if a in node.attrib:
    ...
    def _parse_simple_elements(cls, node, path):
        ...
        for a in cls._ELEMENTS:
            ...
            if not m:
                ...
                if len(vs) == 0 and r:
                    print(f'Missing required subelement(s) of type {t.__name__} when parsing an object of type {cls.__name__}')
...
class URDF(URDFTypeWithMesh):
    ...
    _ELEMENTS = {
        "links": (Link, True, True),                    # An instance of LINK class (type, required, multiple)
        "joints": (Joint, False, True),                 # An instance of joint class. (type, required, multiple)
        "transmissions": (Transmission, False, True),   # An instance of Transmission class. (type, required, multiple)
        "materials": (Material, False, True),           # An instance of material class. (type, required, multiple)
    }
    _TAG = "robot"
